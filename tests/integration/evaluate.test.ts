...
vi.mock("../../src/eval/evaluators/deterministic", () => ({
  DeterministicEvaluator: vi.fn(),
}));

vi.mock("../../src/eval/evaluators/tool-health", () => ({
  ToolTester: vi.fn(),
}));

vi.mock("../../src/eval/evaluators/llm-judge", () => ({
  runLlmJudge: vi.fn(),
}));
...
describe("evaluate", () => {
  ...
  beforeEach(async () => {
    ...
    const runnerModule = await import("../../src/eval/core/runner");
    const deterministicModule = await import(
      "../../src/eval/evaluators/deterministic"
    );
    const toolHealthModule = await import(
      "../../src/eval/evaluators/tool-health"
    );
    const llmJudgeModule = await import("../../src/eval/evaluators/llm-judge");
    const consoleReporterModule = await import(
      "../../src/eval/reporters/console"
    );
