...
import { ConfigSchema, WorkflowSchema } from '../src/eval/config';
...
describe('MCP Evaluation Library', () => {
  ...
  describe('DeterministicEvaluator', () => {
    it('should evaluate a successful workflow', () => {
      ...
      const endToEnd = evaluation.results.find(r => r.metric === 'End-to-End Success');
      expect(endToEnd?.passed).toBe(true);
      
      const toolOrder = evaluation.results.find(r => r.metric === 'Tool Invocation Order');
      expect(toolOrder?.passed).toBe(true);
      
      const toolHealth = evaluation.results.find(r => r.metric === 'Tool Call Health');
